...
def matchTemplate(src, matchSrc, label):
    ...
    tw, th = matchSrc.shape
    print("tw=", tw, "th=", th)
    # tl = (max_loc[0] + th + 2, max_loc[1] + tw + 2)
    # cv2.rectangle(src, max_loc, tl, [0, 0, 0])
    # cv2.putText(src, label, min_loc, fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.1,
    #             color=(240, 230, 0))
    for y in range(len(result)):
        for x in range(len(result[y])):
            if result[y][x] > 0.75:
                cv2.rectangle(img, (x,y), (x + th, y + tw), (0, 0, 0), 2)
                cv2.putText(img, label, (x - 5,y - 5), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5,
                                        color=(0, 0, 0))
    cv2.imshow('001', img)
...
binary = imgThreshold(image)
# # horizontalCut(binary)#
# cv2.imshow('binary', binary)
# verticalCut(binary, "num_1")
# 再读取分割好的图片
match = cv2.imread('num_1.jpg', cv2.COLOR_BGR2GRAY)
match = imgThreshold(match)
cv2.imshow('num_1', match)
print(match)
matchTemplate(binary, match, '1')
